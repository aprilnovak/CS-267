...
% better-initial-guess.txt: original code, compiled with -O3, but a linear starting guess

\begin{figure}[H]
\centering
\includegraphics[width=0.6\linewidth]{../figures/O3-flag.png}
\caption{Runtime as a function of the number of elements with the naive serial code (no compiler flags) and the same code compiled with the {\tt -O3} option, both with the initial naive iterative solution guess of \(a=1.0\) and with a linear interpolation.}
\end{figure}
