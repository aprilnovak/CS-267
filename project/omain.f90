...
! This version extends lmain.f90 by changing the bounds of the domain
! allocated to each process throughout the simulation to obtain faster
! convergence.
...
integer, dimension(:), allocatable :: recv_displs1 ! displacement of each domain
real(8), dimension(:), allocatable :: prev1        ! prev1ious interface values
real(8), dimension(:), allocatable :: soln        ! global solution vector
real(8), dimension(:), allocatable :: xel         ! coordinates in each domain
integer, dimension(:), allocatable :: numnodes1    ! number of nodes in each domain
integer, dimension(:), allocatable :: elems1       ! n_el in each domain
...
integer, dimension(:), allocatable :: recv_displs2 ! displacement of each domain
real(8), dimension(:), allocatable :: prev2        ! prev1ious interface values
integer, dimension(:), allocatable :: numnodes2    ! number of nodes in each domain
integer, dimension(:), allocatable :: elems2       ! n_el in each domain
integer, dimension(:, :), allocatable :: edges2    ! nodes on edge of each domain
...
integer, dimension(:, :), allocatable :: edges1    ! nodes on edge of each domain
...
! initialize variables that are known to all MPI processes and are global
...
! 1 : original equally-partitioned domains
! 2 : alternative domain decomposition
allocate(numnodes1(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of numnodes1 array failed."
allocate(elems1(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of elems1 array failed."
allocate(edges1(2, numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of edges1 array failed."
allocate(recv_displs1(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of recv_displs1 array failed."
allocate(prev1(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of prev1 array failed."

allocate(numnodes2(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of numnodes2 array failed."
allocate(elems2(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of elems2 array failed."
allocate(edges2(2, numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of edges2 array failed."
allocate(recv_displs2(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of recv_displs2 array failed."
allocate(prev2(numprocs), stat = AllocateStatus)
if (AllocateStatus /= 0) STOP "Allocation of prev2 array failed."
...
n_el = elems1(rank + 1)
n_nodes = numnodes1(rank + 1)

allocate(xel(numnodes1(rank + 1)), stat = AllocateStatus)
...
xel = x(edges1(1, rank + 1):edges1(2, rank + 1)) ! domain sizes don't change
...
do while (itererror > ddtol)
  ...
  call mpi_allreduce(abs(BCvals(2) - prev1(2) + BCvals(1) - prev1(1)), itererror, 1, mpi_real8, mpi_sum, &
             mpi_comm_world, ierr)  
...
call mpi_gatherv(a(1:(n_nodes - 1)), n_nodes - 1, mpi_real8, &
                    soln(1:(n_nodes_global - 1)), elems1, recv_displs1, mpi_real8, &
...
deallocate(numnodes1, elems1, edges1, recv_displs1, prev1)
deallocate(numnodes2, elems2, edges2, recv_displs2, prev2)
...
subroutine initializedecomp2()
  implicit none

  ! distribute the elements among the processors. maxperproc should not be too small
  ! such that maxperproc / 2 is less than 1. The first processor is assigned roughly
  ! half the elements it would have obtained with the first discretization.
  elems2(1) = maxperproc / 2
  j = n_el_global - elems2(1) ! number left to divide up
